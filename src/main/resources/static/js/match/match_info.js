...
const modalData = [
    ...
    {
        dialogName: '.info-cancle-dialog',
        openName: 'button.info-cancle-Open',
        closeName: 'button.info-cancle-Close'
    },
    {
...
const insertUserInfo = ($container, users) => {
    ...
    $$matchUserInfo.forEach(($matchUserInfo, index) => {
        ...
        $userProfileImg.src = users[index].images == null ? nullUserImg : users[index].images;
...
const addEvents = () => {
    const $matchCancellationBtn = document.querySelector('.info-cancle-dialog .dialog-button-container button:nth-child(2)');
    $matchCancellationBtn.addEventListener('click', matchCacellation);

    const $matchConfirmedBtn = document.querySelector('.info-confirm-dialog .dialog-button-container button:nth-child(2)');
    $matchConfirmedBtn.addEventListener('click', matchConfirmed);
}

const matchCacellation = async () => {
    const res = await fetch(`/api/matching/cancel/${matchHistoryId}`);
    window.location.href = '/';
}

const matchConfirmed = async () => {
    const res = await fetch(`/api/matching/confirm/${matchHistoryId}`);
    window.location.href = '/';
}
