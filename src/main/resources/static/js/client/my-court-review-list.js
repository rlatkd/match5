...
let pageNum = 1;
let stadiumList;
let userId;  //현재 세션에 있는 유저 ID에는 구장후기 관련 데이터가 없기때문에
...
const getStadiumList = async () => {
    try {
        const res = await fetch(`/api/review/court/detail/all/${userId}/${pageNum}/${itemsPerPage}`);
...
const renderData = (stadiumList) => {
    ...
    for (let i = 0; i < itemsPerPage; i++) {
        ...
        if (item) { //데이터가 있는 경우
            itemElement.html(`
                <div class="index">${formatDate(item.createdAt)}</div>
            ...
            itemElement.click(() => {
                ...
                const courtReviewId = item.courtId;
                const url = `/mypage/my-court-review-list/detail?courtReviewId=${courtReviewId}`;
