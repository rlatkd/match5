...
public interface CourtReviewService {
    // 단일 - 구장평가 생성  [ 프로시저 사용하기위해 Map 사용 ]
    void saveCourtReview(Map<String,Object> paramMap);
    // 단일 - 구장평가 수정 [ 프로시저 사용하기위해 Map 사용 ]
    void updateCourtReviewById(Map<String,Object> paramMap);
    // 단일 - 구장평가 삭제 [ 프로시저 사용하기위해 Map 사용 ]
    void deleteCourtReviewById(Map<String,Object> paramMap);
    // 구장평가 조회 [ 구장평가 정보 ]
    CourtReview findCourtReviewById(Integer courtReviewId);
    ...
    // 유저가 작성한 모든 구장평가 상세 조회   [ 구장평가 정보 + 구장 사진,구장 이름,위도,경도 ]
    List<CourtReviewDetailRes> findAllCourtReviewDetailByIdPaging( Integer userId, Integer page,Integer size);
